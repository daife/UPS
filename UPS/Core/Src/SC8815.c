...
void SC8815_Init_Demo(void)
{
    ...
    SC8815_InterruptMaskInitStruct.AC_OK =0;
    SC8815_InterruptMaskInitStruct.EOC =0;
		SC8815_InterruptMaskInitStruct.INDET =0;
    SC8815_InterruptMaskInitStruct.OTP =0;
    SC8815_InterruptMaskInitStruct.VBUS_SHORT =0;
    ...
    SC8815_HardwareInitStruct.TERM = SCHWI_TERM_Enable;//充够了中断充电
...
HAL_GPIO_WritePin(PSTOP_GPIO_Port,PSTOP_Pin,GPIO_PIN_RESET);
    ...
    SC8815_SetBusCurrentLimit(4950);//VBUS限流
    SC8815_VINREG_SetVoltage(16400);//VBAT电压,设置低一点
		SC8815_SetOutputVoltage(4950);//OUTPUT电压
